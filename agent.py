...
def _sampler_loop():
    ...
    global _THROUGHPUT
    try:
        prev = parse_proc_net_dev(_read_proc_net_dev(), TRAFFIC_DEVICES)
    except Exception:
        prev = {}
    ...
    while True:
        ...
        try:
            cur = parse_proc_net_dev(_read_proc_net_dev(), TRAFFIC_DEVICES)
            t = time.monotonic()
            rates = compute_rates(prev, cur, t - prev_t)
            with _TPUT_LOCK:
                _THROUGHPUT = rates
            prev, prev_t = cur, t
        except Exception:
...
def act_peers(params):
    ...
    for key, b2 in s2.items():
        lip, lport, rip, rport = key
        if not rip or rip.startswith("127.") or rip == "::1":
            continue
        rate = max(0, b2 - s1.get(key, b2)) * 8 / dt   # bit/s over the window
